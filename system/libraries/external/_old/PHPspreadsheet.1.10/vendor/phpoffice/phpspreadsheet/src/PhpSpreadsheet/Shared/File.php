...
class File
{
    ...
    public static function fileExists($pFilename)
    {
        ...
        if (strtolower(substr((string) $pFilename, 0, 3)) == 'zip') {
            // Open ZIP file and verify if the file exists
            $zipFile = substr((string) $pFilename, 6, strpos($pFilename, '#') - 6);
            $archiveFile = substr((string) $pFilename, strpos($pFilename, '#') + 1);
