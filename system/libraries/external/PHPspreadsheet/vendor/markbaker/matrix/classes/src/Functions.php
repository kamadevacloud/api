...
class Functions
{
    ...
    public static function inverse($matrix, string $type = 'inverse')
    {
        ...
        if (!$matrix->isSquare()) {
            throw new Exception(ucfirst((string) $type) . ' can only be calculated for a square matrix');
        }

        $determinant = self::getDeterminant($matrix);
        if ($determinant == 0.0) {
            throw new Div0Exception(ucfirst((string) $type) . ' can only be calculated for a matrix with a non-zero determinant');
        }
