...
class Xlsx extends BaseReader
{
            ...
            if ($mainNS !== '') {
                ...
                if ($xmlWorkbook->sheets) {
                    ...
                    foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
                        ...
                        $fileWorksheetPath = strpos($fileWorksheet, '/') === 0 ? substr((string) $fileWorksheet, 1) : "$dir/$fileWorksheet";
    ...
    private function fileExistsInArchive(ZipArchive $archive, $fileName = ''): bool
    {
        ...
        if (strpos($fileName, '//') !== false) {
            $fileName = substr((string) $fileName, strpos($fileName, '//') + 1);
        }
        ...
        if ($contents === false) {
            $contents = $archive->locateName(substr((string) $fileName, 1), ZipArchive::FL_NOCASE);
        }
    ...
    private function getFromZipArchive(ZipArchive $archive, $fileName = '')
    {
        ...
        if ($contents === false) {
            $contents = $archive->getFromName(substr((string) $fileName, 1), 0, ZipArchive::FL_NOCASE);
        }
                            ...
                            if (!$this->readDataOnly) {
                                ...
                                if ($unparsedVmlDrawings) {
                                    foreach ($unparsedVmlDrawings as $rId => $relPath) {
                                        $rId = substr((string) $rId, 3); // rIdXXX
    ...
    private function readPrinterSettings(Spreadsheet $excel, $dir, $fileWorksheet, $docSheet, array &$unparsedLoadedData): void
    {
        ...
        foreach ($sheetPrinterSettings as $rId => $printerSettings) {
            $rId = substr((string) $rId, 3) . 'ps'; // rIdXXX, add 'ps' suffix to avoid identical resource identifier collision with unparsed vmlDrawing
