        ...
        for ($i = 0; $i < $countProperties; ++$i) {
            ...
            switch ($type) {
                ...
                case 0x40: // Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
                    // PHP-time
                    $value = OLE::OLE2LocalDate(substr((string) $this->summaryInformation, $secOffset + 4 + $offset, 8));
    ...
    private function readDocumentSummaryInformation(): void
    {
        ...
        for ($i = 0; $i < $countProperties; ++$i) {
            ...
            switch ($type) {
                ...
                case 0x40:    //    Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
                    // PHP-Time
                    $value = OLE::OLE2LocalDate(substr((string) $this->documentSummaryInformation, $secOffset + 4 + $offset, 8));
...
//                                                    ->setAuthor($author)
    ...
    private function readTextObject(): void
    {
        ...
        $textStr = substr((string) $text['recordData'], $text['spliceOffsets'][0] + 1, $textByte);
        // get 1 byte
        ...
        if (!$this->readDataOnly) {
            ...
            switch ($hyperlinkType) {
                case 'URL':
                    ...
                    if ($nullOffset) {
                        $url = substr((string) $url, 0, $nullOffset);
                    }
                    $url .= $hasText ? '#' : '';
                    $offset += $us;
                ...
                case 'local':
                    ...
                    $shortenedFilePath = substr((string) $shortenedFilePath, 0, -1); // remove trailing zero
    ...
    private function getFormulaFromData($formulaData, $additionalData = '', $baseCell = 'A1')
    {
        ...
        while (strlen((string) $formulaData) > 0 && $token = $this->getNextToken($formulaData, $baseCell)) {
            $tokens[] = $token;
...
//        $ruleCount = self::getUInt2d($recordData, 0);
        ...
        $cellRangeAddressList = ($this->version == self::XLS_BIFF8)
            ? $this->readBIFF8CellRangeAddressList(substr((string) $recordData, 12))
            : $this->readBIFF5CellRangeAddressList(substr((string) $recordData, 12));
        $cellRangeAddresses = $cellRangeAddressList['cellRangeAddresses'];
    ...
    {
        try {
            $formula = substr((string) $recordData, $offset, $size);
